...
func _on_object_took_damage(object: Node, hit_controller: Node2D, _instigator: Node2D, contact_point: Vector2) -> void:
	...
	print_debug("%s: object=%s hit_controller=%s instigator=%s contact_point=%s" % [name, object.name, hit_controller.name, _instigator.name, contact_point])
