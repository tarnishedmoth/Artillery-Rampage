...
const PARTICLES_GROUP_NAME:StringName = &"Wind_Particles2D"
...
var _active_particles_set: Array = [] ## CPUParticles2D or GPUParticles2D
...
func _ready() -> void:
	...
	for node in get_tree().get_nodes_in_group(PARTICLES_GROUP_NAME):
...
func _apply_wind_to_particles(group:Array) -> void:
	var windspeed:float = wind.x
	for member in group:
		if member is CPUParticles2D:
			if member.gravity.x != windspeed:
				member.gravity.x = windspeed
				
		elif member is GPUParticles2D:
			var process_material:ParticleProcessMaterial = member.process_material
			var member_gravity:Vector3 = process_material.get_gravity()
			
			# Only control the x gravity
			member.process_material.set_gravity(Vector3(windspeed*5, member_gravity.y, member_gravity.z))
...
func _on_particles_group_member_freed(member) -> void: ## Dynamic typed, handles more than one class
